    ...
    <div className="bg-white rounded-lg shadow-md p-6">
      ...
      <div className="space-y-4">
        {statusOrder.map((status, index) => {
          ...
          const statusEntry = statusHistory.find((item) => item.status === status);
          ...
          return (
            <div key={status} className="flex items-start space-x-4">
              ...
              >
                <Icon className={`h-5 w-5 ${isCompleted ? config.color : 'text-gray-400'}`} />
              </div>
              ...
              <div className="flex-1 min-w-0">
                <div className="flex items-center justify-between">
                  ...
                  {statusEntry && (
                    <p className="text-xs text-gray-500">{formatTimestamp(statusEntry.timestamp)}</p>
                  )}
                </div>
