...
const OperatorDashboard = () => {
  ...
  useEffect(() => {
    const loadRequests = async () => {
      ...
      setError('');
      try {
        const { requests: apiRequests } = await fetchRequests();
  ...
  const handleStatusUpdate = async (requestId, newStatus) => {
    try {
      const { request } = await updateRequestStatus(requestId, { status: newStatus });
      setRequests((prev) => prev.map((item) => (item.id === request.id ? request : item)));
      setSelectedRequest((prev) => (prev?.id === request.id ? request : prev));
    } catch (err) {
    ...
    <div className="min-h-screen bg-gray-50">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <div className="bg-white rounded-lg shadow-md overflow-hidden">
          ...
          {isLoading ? (
            <div className="px-6 py-8 text-center text-gray-600">Loading requests...</div>
          ) : (
